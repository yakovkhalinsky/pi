...
- **TUI tools (preferred over raw bash):** the `agentic-team-protocol` extension (`~/.pi/agent/extensions/agentic-team-protocol/index.ts`) registers `team_status`, `team_recall`, `team_remember`, `team_lookup`, `team_decide`, and `team_purge` tools that wrap the same Eden-memory CLI and render themed TUI output (state-pill tables, scored match lists, confirmation/record cards, decision cards). Role subagents and the parent assistant should call these tools instead of `bash` + `eden.sh` for ATP memory I/O so the tool-call output is rich and auditable. The `/team-board` command renders a full-width bordered goal board (`/team-board all` includes closed rows); `/team-approve` is the human-decision flow for `pending_authorisation`/`blocked` goals. If the extension is not loaded, fall back to `bash` + `eden.sh`.
...
- **Goal-board widget.** While a team runs, a widget above the editor always shows: the goal board (state/stage/owner per goal; while a hand-off is in flight the owner column shows the incoming role as `→ role` — roles write their stage record at the END of their work, so the handing-off role would otherwise own the board until the next role finishes), **running agents attached as activity lines under the goal they are working** (`  ● → edit runner.ts · 14 turns · 6m` — the role name is the goal row's OWNER column and is not repeated; nested child runs indent with `↳`; ambiguous matches render standalone with the role name; a running agent whose last transcript write is a thinking stub with no transcript churn for 3+ min renders `⚠ stalled? · silent Xm` instead of a growing elapsed timer — long single tool calls (`→ tool`), tool-result states (`✓/✗ tool`) and text replies keep the normal elapsed display), and the needs-you list + queued steers. Completed runs are dropped — the board's goal rows carry terminal state; failed/aborted runs surface transiently as ✗ error lines for 2 min (or until a retry of the same role spawns) so a died child is never silent. **Closed goals stop rendering as rows** (they are summarized in a dim hidden-count line so live state stays visible; `/team-board all` shows them, `/team-purge` removes them). It updates from subagent run events plus a 1s poller over the session's `.subagents/manifest.jsonl` and child session transcripts; the footer status line summarizes `ATP <n> active · <m> pending`.
...
Closed goals accumulate in Eden-memory forever otherwise — cluttering the board, `team_status`, and recall long after their audit value has been extracted. Two-layer answer (2026-09-06):

1. **Presentation (no memory touched):** closed goals are terminal state and stop rendering as board rows on the widget, `/team-board` (default view), and `team_status` (default view); a dim hidden-count line replaces them. Stranded pending items on closed goals still surface in the needs-you list. `/team-board all` / `include_closed: true` opt back into the full table; an explicit `goal_id` filter always includes its goal (role rehydration is unaffected).
2. **Durable removal (explicit, never automatic):** `team_purge` (tool) / `/team-purge` (command) forgets a closed goal's records under the F1 cleanup discipline:
   - Only goals whose state is `closed` are purgeable; a goal with an unresolved `pending_authorisation`/`blocked` record is refused (closure disposition rule — decide via `team_decide` first).
   - Every record id is enumerated by the goal-scoped metadata scan, then per-id `lookup`-verified against the goal before any forget (F1 point 3). Anything that fails verification is retained.
   - A purge-audit `cleanup_record` (owner `archivist`) is stored BEFORE the forgets and edited with the actual outcome after (F1 point 4). It carries NO `goal_id` metadata key and NO `Goal:` identity line — either would resurrect the purged goal as a board row.
   - `forget` is a soft delete (recoverable from the DB until a real prune); the audit record is always retained.
   - `dry_run: true` (or bare `/team-purge`) previews; `all: true` targets every purgeable closed goal in the workspace. Bulk removal is bounded to closed goals — never workspace/agent-scope-wide.
...
- **Stale closures** — a new action record after closure supersedes it; do not assume an old `archival_record` is the final word. Post-closure run_logs and hand-offs at ANY stage do NOT reopen the goal — the board keeps rendering it closed (an in-flight work-stage run_log stays visible as the interrupted-work signal). Only a new `action_record`, a verdict, a rework dispatch, an escalation, an authorisation, or a cleanup record reopens it.
...
- `/team-status` — list open goals, current stage, owner role, latest record IDs, and continueable/blocked state (closed goals are summarized in a hidden-count line unless `include_closed` is set).
...
- `/team-purge` — preview (`/team-purge`) or durably remove (`/team-purge <goal-id>` / `/team-purge all`) closed goals from Eden-memory; see "Closed-goal removal" below.
