...
- **Stale closures** — a new action record after closure supersedes it; do not assume an old `archival_record` is the final word. Post-closure router bookkeeping (run_logs / closure hand-offs at stage `hand_off_or_closure`) does NOT reopen the goal — the board keeps rendering it closed; only a new `action_record`, a verdict, or a rework dispatch does.
